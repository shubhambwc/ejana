...
@section('content')
    <section class="section">
        <div class="section-header flex-wrap">
            <h1 class="mb-2">{{ __('Services') }}</h1>
            <div class="section-header-breadcrumb">
                @if(count($jobCategories) > 0)
                    <div class="card-header-action grid-flex-end">
                        {{  Form::select('is_featured', $featured, null, ['id' => 'filterFeatured', 'class' => 'form-control status-filter w-100', 'placeholder' => 'Select Active Job']) }}
                    </div>
                @endif
                <a href="#"
