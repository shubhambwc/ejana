...
@section('title')
    {{ html_entity_decode($companyDetail->user->first_name) }}
@endsection
@section('content')
            ...
            <div class="row company-profile">
                <div class="col-md-3 col-xs-12">
                    <div class="profile-photo company-detail-logo ticket-sender-picture">
                        <img src="{{ (!empty($companyDetail->company_url)) ? $companyDetail->company_url : asset('assets/img/logo-dark.png') }}"
    ...
    border: 1px solid #1cd3c1;">Apply</a></div>
                        ...
                        <ul class="profile-info mt20 nopadding">
                            ...
                            @if(!empty(@$postMetaArray['address']))
                                <li>
                                    <i class="fa fa-map-marker"></i>
                                    <span>
                                            {{ (!empty(@$postMetaArray['address'])) ? @$postMetaArray['address'].', ' : '' }}
                                        {{ (!empty(@$postMetaArray['zip_code'])) ? @$postMetaArray['zip_code'].', ' : '' }}
                        ...
                        <ul class="profile-info mt20 nopadding">
                            @if(!empty(@$postMetaArray['date_childcare_need']))
                                <li style="width: 290px;">
                                    <i class="fa fa-calendar"></i>
                                    <span><strong>Need Child Care on :</strong> {{ @$postMetaArray['date_childcare_need'] }}</span>
                                </li>
                             @endif
                              @if(!empty(@$postMetaArray['type_of_care']))
                                <li style="width: 290px;">
                                    <i class="fa fa-address-card-o  
"></i>
                                    <span><strong>Type of Care :</strong> {{ @$postMetaArray['type_of_care'] }}</span>
                                </li>
                             @endif
                        </ul>
                         <ul class="profile-info mt20 nopadding">
                            @if(!empty(@$postMetaArray['minimum_exp']))
                                <li style="width: 290px;">
                                    ...
                                    <span><strong>Experience Req. :</strong> {{ @$postMetaArray['minimum_exp'] }} - {{ @$postMetaArray['maximum_exp'] }} Years</span>
                                </li>
                             @endif
                              @if(!empty(@$postMetaArray['minimum_age']))
                                <li style="width: 290px;">
                                    <i class="fa fa-child"></i> 

                                    <span><strong>Age Req. :</strong> {{ @$postMetaArray['minimum_age'] }} - {{ @$postMetaArray['maximum_age'] }} Years</span>
                                </li>
                             @endif

                        </ul>
                        <ul class="profile-info mt20 nopadding">
                             @if(!empty(@$postMetaArray['other_wish']))
                                <li style="width: 290px;">
                                    <i class="fa fa-info"></i>   

                                    <span><strong>Other Wish :</strong> {{ @$postMetaArray['other_wish'] }} </span>
                                </li>
                             @endif
                             @if(!empty(@$postMetaArray['remarks']))
                                <li style="width: 290px;">
                                    <i class="fa fa-info-circle"></i>   

                                    <span><strong>Remarks :</strong> {{ @$postMetaArray['remarks'] }} </span>
                                </li>
                             @endif
                        </ul>
                          <ul class="profile-info mt20 nopadding">
                             @if(!empty(@$postMetaArray['advertisement']))
                                <li style="width: 290px;">
                                    <i class="fa fa-audio-description   
"></i>   

                                    <span><strong>Advertisement :</strong> {{ @$postMetaArray['advertisement'] }} </span>
                                </li>
                             @endif
                             @if(!empty(@$postMetaArray['advertisement_text']))
                                <li style="width: 290px;">
                                    <i class="fa fa-audio-description   
"></i>   

                                    <span><strong>Advertisement Text :</strong> {{ @$postMetaArray['advertisement_text'] }} </span>
                                </li>
                             @endif
                        </ul>
                        @auth
