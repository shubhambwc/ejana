...
$(document).on('submit', '#addCandidateNewForm', function (e) {
  ...
  if (isGoogleReCaptchaEnabled) {
    if (!checkGoogleReCaptcha(1)) return true;
  }
  ...
  $.ajax({
    ...
    console.log(result)
      ...
      $('#owner_id').val(result.message.ownerId);
      
      $('#new_user_form').submit()
       setTimeout(function () {
       	$('#new_user_form').submit()
       	//window.location = afterRegisterURL+'/'+result.message.ownerId;
        }, 500);
...
$(document).on('submit', '#addEmployerNewForm', function (e) {
  ...
  if (isGoogleReCaptchaEnabled) {
    if (!checkGoogleReCaptcha(2)) return true;
  }
...
module.exports = __webpack_require__(/*! C:\EjanaPortal\resources\assets\js\front_register\front_register.js */"./resources/assets/js/front_register/front_register.js");
