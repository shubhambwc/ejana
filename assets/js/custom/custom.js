...
window.isValidFile1 = function (inputSelector, validationMessageSelector) {
  var ext = $(inputSelector).val().split('.').pop().toLowerCase();
 
  if ($.inArray(ext, ['gif', 'png', 'jpg', 'jpeg','pdf','docx','txt']) == -1) {
    $(inputSelector).val('');
    $(validationMessageSelector).removeClass('d-none');
    $(validationMessageSelector).html('The selected must be a file of type: jpeg, jpg, png ,pdf.').show();
    $(validationMessageSelector).delay(5000).slideUp(300);
    return false;
  }
...
window.displayPdf = function (input, selector) {
  ...
  if (input.files && input.files[0]) {
    ...
    reader.onload = function (e) {
      var image = new Image();
      image.src = 'https://www.ejana.nl/assets/img/logo-dark.png';
      image.onload = function () {
       $(selector).attr('src', 'img/pdf.png');
        displayPreview = true;
      };
    };
...
module.exports = __webpack_require__(/*! C:\EjanaPortal\resources\assets\js\custom\custom.js */"./resources/assets/js/custom/custom.js");
