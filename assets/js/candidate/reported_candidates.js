...
$(document).on('click', '.delete-btn', function (event) {
  ...
  swal({
    ...
    window.livewire.emit('deleteReportedCandidate', reportedCandidateId);
  });
});
  ...
  $.ajax({
    ...
    success: function success(result) {
      if (result.success) {
        ...
        if (!isEmpty(result.data.note) ? $('#showNote').append(result.data.note) : $('#showNote').append('N/A')) $('#showName').append(result.data.candidate.user.first_name);
        $('#showReportedBy').append(result.data.user.first_name);
        $('#showReportedOn').append(result.data.date);
        $('#showImage').append('<img src="' + result.data.candidate.candidate_url + '" class="img-responsive users-avatar-img employee-img mr-2" />');
...
module.exports = __webpack_require__(/*! C:\EjanaPortal\resources\assets\js\candidate\reported_candidates.js */"./resources/assets/js/candidate/reported_candidates.js");
