...
class CompanyRepository extends BaseRepository
{
    ...
    public function prepareData()
    {
        ...
        $data['industries'] = JobCategory::pluck('name', 'id');
        $data['language'] = Language::pluck('language', 'id');
        $data['lang_level'] = Langauge_levels::pluck('name', 'id');
    ...
    public function store($input)
    {
        ...
        try {
            ...
            $input['language'] = $input['lang_id'];
            $input['lang_level'] = $input['level_id'];
            ...
            $userInput = Arr::only($input,
                [
                   ...
                   'first_name', 'email', 'phone', 'password', 'owner_id', 'owner_type', 'country_id', 'state_id',
                    ...
                    'region_code','language','lang_level'
                ]);
            // dd($userInput);
    ...
    public function update($input, $company)
    {
        try {
            ...
            $userInput = Arr::only($input, 
                [
                   'first_name','last_name', 'email', 'phone', 'password', 'country_id', 'state_id', 'city_id', 'is_active',
                     'region_code','language','lang_level','service_id'
                ]);
            /** @var User $user */
            $user = $company->user;
    ...
    public function get($input = [])
    {
        ...
        $subQuery->map(function (Company $company) use ($data, &$result) {
            ...
            $data['user'] = [
                ...
                'service_id'   => $company->user->service_id,
                'owner_type'   => $company->user->owner_type,
            ];
