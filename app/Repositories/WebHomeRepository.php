...
class WebHomeRepository
{
    ...
    public function getFeaturedCompanies()
    {
        return Company::has('activeFeatured')
            ->with(['jobs', 'activeFeatured', 'user.media'])
     ...
     */
     ...
     public function storeComplaintus($input)
    {
       
        /** @var Inquiry $inquiry */
        $complaint = Complaint::create($input);
        $templateBody = EmailTemplate::whereTemplateName('Contact Us')->first();
        $keyVariable = ["{{name}}",  "{{from_name}}"];
        $value = [$complaint->name, config('app.name')];
        $body = str_replace($keyVariable, $value, $templateBody->body);
        $data['inquiry'] = $complaint;
        $data['subject'] ='Complaint againest '.$complaint->candidate_customer;
        $data['body'] = $body;
        Mail::to('info.ejana@gmail.com')->send(new ContactEmail($data));
    ...
    public function storeInquires($input)
    {
        ...
        $templateBody = EmailTemplate::whereTemplateName('Contact Us')->first();
        $keyVariable = ["{{name}}",  "{{from_name}}"];
        $value = [$inquiry->name, config('app.name')];
        $body = str_replace($keyVariable, $value, $templateBody->body);
        $data['inquiry'] = $inquiry;
        $data['subject'] ='Get In Touch Request';
        $data['body'] = $body;
        Mail::to('info.ejana@gmail.com')->send(new ContactEmail($data));
    ...
    {
        return $blog = Post::with('postAssignCategories', 'media', 'user')
