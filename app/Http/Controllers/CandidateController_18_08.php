...
class CandidateController extends AppBaseController
{
    ...
    public function update(Candidate $candidate, UpdateCandidateRequest $request)
    {
       ...
       if ((isset($input['image1']))) {
                $candidate->clearMediaCollection('data-thumbnail1');
                $candidate->addMedia($input['image1'])->toMediaCollection('data-thumbnail1');
         }

        if ((isset($input['image2']))) {
                $candidate->clearMediaCollection('data-thumbnail2');
                $candidate->addMedia($input['image2'])->toMediaCollection('data-thumbnail2');
         }

         if ((isset($input['image3']))) {
                $candidate->clearMediaCollection('data-thumbnail3');
                $candidate->addMedia($input['image3'])->toMediaCollection('data-thumbnail3');
         }
