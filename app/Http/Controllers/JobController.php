...
class JobController extends AppBaseController
{
    ...
    public function index(Request $request)
    {
        ...
        if ($request->ajax()) {
            ...
            return Datatables::of((new JobDataTable())->get());
        }
        ...
        $job_applications = DB::table('users')
            ...
            ->select('job_applications.*', 'users.*', 'job_categories.name','job_applications.id as job_applications_id')
            ->where('job_id',  Auth::user()->owner_id)
            ->get();
        ...
        return view('employer.jobs.index',
            compact('statusArray', 'isFeaturedEnable', 'isFeaturedAvilabal', 'featured','job_applications'));
    }
    ...
    public function editjob_status(Request $request)
    {
        $id = request('job_applications_id');
        $status = request('status');
        
        $editjob_status = JobApplication::find($id)->update([
            
            'status' => request('status')
            ]);
            
            
                 return redirect()->route('job.index')->with('success', 'Status updated successfully.');   
           
 
    }
     public function destroy_job_app($id)

    {
        
       $jobdel = JobApplication::find($id);
        $jobdel->delete();
       return $this->sendSuccess('Job deleted successfully.');
